...
async function showBonus(ctx) {
  const caption = ['<b>Cosmo Bonus</b>', 'Простая loyalty-механика для MVP.', '', '🎁 <code>COSMO10</code> — 10% на первый заказ', '👥 <code>FRIENDS15</code> — 15% от 2 позиций', '☕ <code>7THCUP</code> — каждый 7-й кофе бесплатно'].join('\n');
  await safePhoto(ctx, 'https://images.unsplash.com/photo-1442512595331-e89e73853f31?auto=format&fit=crop&w=1400&q=80', caption, Markup.inlineKeyboard([[Markup.button.callback('Открыть меню', 'menu'), Markup.button.callback('Корзина', 'cart')]]));
}
...
async function finishOrder(ctx, phone) {
  ...
  const managerText = ['New Cosmo demo order', `Guest: ${name}`, `Phone: ${phone}`, `Branch: ${branch}`, '', orderLines, '', `Total: ${money(total)}`].join('\n');
  if (managerChatId) await ctx.telegram.sendMessage(managerChatId, managerText);
  await ctx.reply(['✅ <b>Заказ принят</b>', `Гость: <b>${html(name)}</b>`, `Филиал: <b>${html(branch)}</b>`, `Итого: <b>${money(total)}</b>`, '', 'Для MVP это показывает полный сценарий: меню → корзина → заявка менеджеру.'].join('\n'), { parse_mode: 'HTML', ...mainKeyboard });
  ctx.session.cart = [];
