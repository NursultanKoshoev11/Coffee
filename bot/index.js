...
const branches = [
  { id: 'pickup', title: 'Pickup preorder', time: '08:00–22:00', note: 'кофе и еда без ожидания' },
  { id: 'dinein', title: 'Dine-in', time: '08:00–22:00', note: 'заказ для гостей внутри Focus' },
  { id: 'booking', title: 'Table booking', time: '10:00–22:00', note: 'бронь стола под завтрак, обед или ужин' }
];
...
const products = [
  ...
  { id: 'filter', code: 'FOCUS-BATCH', category: 'coffee', title: 'Batch Brew', price: 160, short: 'фильтр дня, мягкий профиль, to go', image: 'https://images.unsplash.com/photo-1442512595331-e89e73853f31?auto=format&fit=crop&w=1200&q=80' },
  { id: 'toast', code: 'FOCUS-TOAST', category: 'kitchen', title: 'Avocado Toast', price: 290, short: 'sourdough, avocado, egg, greens', image: 'https://images.unsplash.com/photo-1525351484163-7529414344d8?auto=format&fit=crop&w=1200&q=80' },
  { id: 'breakfast', code: 'FOCUS-BRUNCH', category: 'kitchen', title: 'Breakfast Plate', price: 340, short: 'яйца, салат, тост, соус, сезонные добавки', image: 'https://images.unsplash.com/photo-1533089860892-a7c6f0a88666?auto=format&fit=crop&w=1200&q=80' },
  { id: 'shakshuka', code: 'FOCUS-SHAK', category: 'kitchen', title: 'Shakshuka', price: 320, short: 'томаты, яйца, специи, хлеб', image: 'https://images.unsplash.com/photo-1590412200988-a436970781fa?auto=format&fit=crop&w=1200&q=80' },
  { id: 'sandwich', code: 'FOCUS-SAND', category: 'kitchen', title: 'Chicken Sandwich', price: 310, short: 'курица, соус, зелень, хрустящий хлеб', image: 'https://images.unsplash.com/photo-1553909489-cd47e0907980?auto=format&fit=crop&w=1200&q=80' },
  { id: 'croissant', code: 'FOCUS-CRO', category: 'dessert', title: 'Butter Croissant', price: 110, short: 'тёплый круассан к утреннему кофе', image: 'https://images.unsplash.com/photo-1555507036-ab1f4038808a?auto=format&fit=crop&w=1200&q=80' },
  { id: 'cheesecake', code: 'FOCUS-CAKE', category: 'dessert', title: 'Burnt Cheesecake', price: 190, short: 'нежный десерт к фильтру или латте', image: 'https://images.unsplash.com/photo-1533134242443-d4fd215305ad?auto=format&fit=crop&w=1200&q=80' }
];
...
const categories = {
  ...
  kitchen: { title: '🍽 Kitchen', ids: products.filter(p => p.category === 'kitchen').map(p => p.id) },
  dessert: { title: '△ Dessert', ids: products.filter(p => p.category === 'dessert').map(p => p.id) }
};
...
const mainKeyboard = Markup.keyboard([['☕ Coffee', '🍽 Kitchen'], ['🛒 Корзина', '📅 Бронь'], ['🏆 Focus League', '🧾 Предзаказ']]).resize();
const menuKeyboard = Markup.inlineKeyboard([[Markup.button.callback('● Популярное', 'cat:popular'), Markup.button.callback('☕ Coffee', 'cat:coffee')], [Markup.button.callback('🍽 Kitchen', 'cat:kitchen'), Markup.button.callback('△ Dessert', 'cat:dessert')], [Markup.button.callback('📅 Бронь', 'reserve'), Markup.button.callback('🏆 League', 'leaderboard')]]);
...
async function welcome(ctx) { await safePhoto(ctx, 'https://images.unsplash.com/photo-1511920170033-f8396924c348?auto=format&fit=crop&w=1400&q=80', '<b>Focus Coffee & Kitchen</b>\nMenu, kitchen preorder, table booking and loyalty league.', Markup.inlineKeyboard([[Markup.button.callback('Открыть меню', 'menu'), Markup.button.callback('Бронь столика', 'reserve')], [Markup.button.callback('Focus League', 'leaderboard'), Markup.button.callback(`Корзина ${cartQty(ctx)}`, 'cart')]])); await ctx.reply('Главное меню:', mainKeyboard); }
...
async function showBranches(ctx) { const text = ['<b>Focus форматы</b>', ...branches.map(b => `📍 <b>${html(b.title)}</b>\n<code>${html(b.time)}</code>\n${html(b.note)}`)].join('\n\n'); await ctx.reply(text, { parse_mode: 'HTML' }); }
async function showBonus(ctx) { await ctx.reply('<b>Focus Bonus</b>\n<code>FOCUS10</code> — 10% на первый preorder\n<code>KITCHEN15</code> — 15% от 2 позиций\n🏆 Focus League — кофе и кухня дают очки', { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('Меню', 'menu'), Markup.button.callback('League', 'leaderboard')]]) }); }
...
async function showCart(ctx) { if (!ctx.session.cart.length) return ctx.reply('🛒 <b>Корзина пустая</b>\nОткройте меню и добавьте кофе или блюдо.', { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('Меню', 'menu')]]) }); const lines = ctx.session.cart.map(line => { const p = productById(line.id); return `• <b>${html(p.title)}</b> × ${line.qty} — <code>${money(p.price * line.qty)}</code>`; }); await ctx.reply(['🛒 <b>Focus preorder</b>', ...lines, '', `Итого: <b>${money(cartTotal(ctx))}</b>`].join('\n'), { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('Оформить', 'checkout')], [Markup.button.callback('Добавить ещё', 'menu'), Markup.button.callback('Очистить', 'clear')]]) }); }
async function checkout(ctx) { if (!ctx.session.cart.length) return showCart(ctx); ctx.session.step = 'pickup'; await ctx.reply('Напишите pickup/dine-in time. Например: <code>15:30</code> или <code>на месте</code>', { parse_mode: 'HTML' }); }
...
async function finishOrder(ctx, pickup) { const total = cartTotal(ctx); const cups = cartQty(ctx); const rank = await addScore(ctx, { cups, total }); const lines = ctx.session.cart.map(line => { const p = productById(line.id); return `• ${p.title} × ${line.qty}`; }).join('\n'); if (managerChatId) await ctx.telegram.sendMessage(managerChatId, `New Focus Coffee & Kitchen preorder\nGuest: ${publicName(ctx)}\nTime: ${pickup}\n\n${lines}\n\nTotal: ${money(total)}`); await ctx.reply(`✅ <b>Предзаказ принят</b>\nВремя: <b>${html(pickup)}</b>\nИтого: <b>${money(total)}</b>\n\n🏆 Вы теперь #${rank} в Focus League.`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('Показать League', 'leaderboard'), Markup.button.callback('Новое меню', 'menu')]]) }); ctx.session.cart = []; ctx.session.step = null; }
async function finishReserve(ctx, details) { if (managerChatId) await ctx.telegram.sendMessage(managerChatId, `New Focus table booking\nGuest: ${publicName(ctx)}\nDetails: ${details}`); await ctx.reply(`📅 <b>Бронь сформирована</b>\n${html(details)}\n\nМенеджер сможет подтвердить столик.`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('Меню', 'menu'), Markup.button.callback('League', 'leaderboard')]]) }); ctx.session.step = null; }
bot.start(welcome); bot.command('leaderboard', showLeaderboard); bot.hears('☕ Coffee', async ctx => showCategory(ctx, 'coffee')); bot.hears('🍽 Kitchen', async ctx => showCategory(ctx, 'kitchen')); bot.hears('📅 Бронь', reserve); bot.hears('🎁 Bonus', showBonus); bot.hears('🛒 Корзина', showCart); bot.hears('🏆 Focus League', showLeaderboard); bot.hears('🧾 Предзаказ', checkout);
